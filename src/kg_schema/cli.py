...
def run_migrate(db_path: str | None) -> int:
    ...
    db = kg_schema.connect(path)
    try:
        applied = kg_schema.ensure(db, run_migrations=True)
        rows = db.execute(
...
def run_coverage(  # noqa: PLR0913 - keyword-only report knobs, all with defaults
    db_path: str | None,
    *,
    ...
    db = kg_schema.connect(fin_path)
    try:
        kg_schema.ensure(db)  # additive only -- makes `universe_coverage` exist
        uni_db = connect_ro(uni_path)
        try:
            report = check_coverage(
                db, uni_db, as_of, universe=universe, min_observation_days=min_observation_days
            )
        finally:
            uni_db.close()
        written = persist_coverage(db, report)
    finally:
        db.close()
