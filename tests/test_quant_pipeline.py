...
def test_optimize_persists_one_book_per_objective(seeded: Database) -> None:
    ...
    assert queries.current_version(seeded) == 0


def test_optimize_respects_caps(seeded: Database) -> None:
    as_of = seeded.execute("SELECT MAX(obs_date) FROM quant_return_daily").fetchone()[0]
...
def test_optimize_auto_builds_risk_model_when_missing(seeded: Database) -> None:
    as_of = seeded.execute("SELECT MAX(obs_date) FROM quant_return_daily").fetchone()[0]
